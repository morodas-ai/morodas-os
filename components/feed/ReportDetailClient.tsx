...
const statusOptions = [
    ...
    { id: "done", label: "完了", color: "bg-primary-100 text-primary-700" },
    { id: "archived", label: "アーカイブ", color: "bg-surface-100 text-foreground" },
];
    ...
    return (
        <div className="animate-in">
            ...
            <div className="flex items-center justify-between mb-6">
                <Link href="/feed" className="flex items-center gap-2 text-surface-500 hover:text-sidebar-hover transition-colors">
                    <ArrowLeft size={20} />
                    <span>フィードに戻る</span>
                </Link>
                        ...
                        <button
                            ...
                            className={clsx(
                                ...
                                viewMode === "output"
                                    ...
                                    : "text-surface-500 hover:text-sidebar-hover"
                            )}
                        >
                            <FileText size={14} />
                            Output
            ...
            {viewMode === "detail" && (
                <div className="mb-8 space-y-6">
                    <div className="bg-surface-50 rounded-xl p-6 border border-surface-200">
                        <h3 className="text-lg font-bold text-sidebar mb-4 flex items-center gap-2">
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-6">
                            <div className="bg-white rounded-lg p-4 border border-surface-200">
                                ...
                                <div className="font-semibold text-sidebar">{report.agent.name}</div>
                            </div>
                            <div className="bg-white rounded-lg p-4 border border-surface-200">
                                <div className="flex items-center gap-2 text-surface-500 text-sm mb-1">
                                    <Clock size={14} />
                                ...
                                <div className="font-semibold text-sidebar">
                        ...
                        {runDetails?.toolsUsed && runDetails.toolsUsed.length > 0 && (
                            <div className="mb-6">
                                <h4 className="text-sm font-medium text-foreground mb-2">使用されたツール</h4>
                        ...
                        {/* プロンプト表示 */}
                        <div>
                            <h4 className="text-sm font-medium text-foreground mb-2">使用されたプロンプト</h4>
                            <div className="bg-sidebar rounded-lg p-4 overflow-x-auto">
                                <pre className="text-sm text-surface-100 whitespace-pre-wrap font-mono">
                ...
                <div className="flex gap-8">
                    ...
                    <article className="flex-1 max-w-4xl">
                        ...
                        <div className="mb-8 pb-6 border-b border-surface-200">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <div className="relative">
                                    ...
                                    {isStatusOpen && (
                                        <div className="absolute left-0 top-9 bg-white border border-surface-200 rounded-lg shadow-lg py-1 z-10 w-40">
                                            {statusOptions.map((option) => (
                                                <button
                                                    key={option.id}
                                                    onClick={() => updateStatus(option.id)}
                                                    className={`w-full px-4 py-2 text-left text-sm hover:bg-surface-50 flex items-center gap-2 ${option.id === status ? "bg-surface-50" : ""
                                                        }`}
                                                >
                        ...
                        {content.summary && (
                            <section id="summary" className="mb-10">
                                <h2 className="text-xl font-bold text-foreground mb-4">エグゼクティブ・サマリー</h2>
                                <p className="text-sidebar-hover leading-relaxed bg-surface-50 p-4 rounded-lg border-l-4 border-primary-500">
                                    {content.summary}
                                </p>
                            </section>
                        )}
                        ...
                        {content.topics && content.topics.length > 0 && (
                            <section id="topics" className="mb-10">
                                <h2 className="text-xl font-bold text-foreground mb-4">主要トピックと背景分析</h2>
                                <div className="space-y-6">
                                    {content.topics.map((topic, i) => (
                                        <div key={i} className="card p-5">
                                            <h3 className="font-bold text-sidebar mb-2">{topic.title}</h3>
                        ...
                        {content.insights && content.insights.length > 0 && (
                            <section id="insights" className="mb-10">
                                <h2 className="text-xl font-bold text-foreground mb-4">戦略的インサイトと推奨アクション</h2>

                                <div className="overflow-x-auto mb-6">
                                    <table className="w-full border-collapse">
                                        <thead>
                                            <tr className="bg-surface-100">
                                                <th className="text-left p-3 font-semibold text-sidebar-hover border border-surface-200">重点領域</th>
                                                <th className="text-left p-3 font-semibold text-sidebar-hover border border-surface-200">MORODAS戦略</th>
                                                <th className="text-left p-3 font-semibold text-sidebar-hover border border-surface-200">期待効果</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {content.insights.map((insight, i) => (
                                                <tr key={i} className="hover:bg-surface-50">
                                                    <td className="p-3 border border-surface-200 font-medium">{insight.area}</td>
                                                    <td className="p-3 border border-surface-200">{insight.strategy}</td>
                                                    <td className="p-3 border border-surface-200">{insight.expected}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                    ...
                                    <div className="bg-primary-50 p-4 rounded-lg border border-primary-200">
                                        ...
                                        <ul className="space-y-2">
                                            {content.recommendedActions.map((action, i) => (
                                                <li key={i} className="flex items-center justify-between text-primary-700">
                                                    ...
                                                    <button
                                                        ...
                                                        className="text-xs text-primary-600 hover:text-emerald-800 underline"
                                                    >
                                                        タスクに追加
                                                    </button>
                                                </li>
                        ...
                        {content.sns && content.sns.length > 0 && (
                            <section id="sns" className="mb-10">
                                <h2 className="text-xl font-bold text-foreground mb-4">SNS・ニュース動向 (X/Twitter)</h2>
                                <div className="space-y-4">
                                    {content.sns.map((post, i) => (
                                        <div key={i} className="card p-4 border-l-4 border-blue-400">
                                            <div className="flex items-center gap-2 mb-2">
                                                <div className="w-8 h-8 rounded-full bg-surface-200 flex-center">
                                                    <span className="text-xs font-bold">X</span>
                                                </div>
                                                <span className="font-semibold text-sidebar">{post.author}</span>
                                            </div>
                                            <p className="text-sidebar-hover mb-3">{post.content}</p>
                        ...
                        {content.sources && content.sources.length > 0 && (
                            <section id="sources" className="mb-10">
                                <h2 className="text-xl font-bold text-foreground mb-4">情報ソース ({content.sources.length})</h2>
                                <div className="flex flex-wrap gap-3">
                                    {content.sources.map((source, i) => (
                                        <a
                                            ...
                                            className="flex items-center gap-2 px-3 py-2 bg-surface-100 hover:bg-surface-200 rounded-lg text-sm text-sidebar-hover transition-colors"
                                        >
                                            <LinkIcon size={14} />
                                            {source.name}
                    ...
                    <aside className="hidden xl:block w-64 shrink-0">
                        <div className="sticky top-8">
                            <h3 className="font-bold text-sidebar-hover mb-4">目次</h3>
                            <nav className="space-y-2">
                                {toc.map((item) => (
                                    <a
                                        key={item.id}
                                        href={`#${item.id}`}
                                        className="block text-sm text-surface-500 hover:text-primary-600 transition-colors py-1 border-l-2 border-transparent hover:border-primary-500 pl-3"
                                    >
                                        {item.label}
                                    </a>
                                ))}
